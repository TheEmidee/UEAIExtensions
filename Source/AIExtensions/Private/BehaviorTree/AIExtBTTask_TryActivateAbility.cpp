#include "BehaviorTree/AIExtBTTask_TryActivateAbility.h"
...
UAIExtBTTask_TryActivateAbility::UAIExtBTTask_TryActivateAbility( const FObjectInitializer & object_initializer ) :
    Super( object_initializer )
{
    NodeName = "Try Activate Ability";

    BlackboardKey.AddObjectFilter( this, GET_MEMBER_NAME_CHECKED( UAIExtBTTask_TryActivateAbility, BlackboardKey ), AActor::StaticClass() );
...
void UAIExtBTTask_TryActivateAbility::InitializeFromAsset( UBehaviorTree & asset )
{
...
EBTNodeResult::Type UAIExtBTTask_TryActivateAbility::ExecuteTask( UBehaviorTreeComponent & owner_comp, uint8 * node_memory )
{
    ...
    auto * memory = CastInstanceNodeMemory< FAIExtTryActivateAbilityBTTaskMemory >( node_memory );
...
uint16 UAIExtBTTask_TryActivateAbility::GetInstanceMemorySize() const
{
    return sizeof( FAIExtTryActivateAbilityBTTaskMemory );
}

FString UAIExtBTTask_TryActivateAbility::GetStaticDescription() const
{
...
void UAIExtBTTask_TryActivateAbility::TickTask( UBehaviorTreeComponent & owner_comp, uint8 * node_memory, float delta_seconds )
{
    ensure( GetSpecialNodeMemory< FBTTaskMemory >( node_memory )->NextTickRemainingTime <= 0.f );

    const auto * memory = CastInstanceNodeMemory< FAIExtTryActivateAbilityBTTaskMemory >( node_memory );
...
void UAIExtBTTask_TryActivateAbility::OnGameplayTaskDeactivated( UGameplayTask & task )
{
    ...
    if ( const auto * ai_task = Cast< UAIExtAITask_ActivateAbility >( &task ) )
    {
        if ( ai_task->GetAIController() && ai_task->GetState() != EGameplayTaskState::Paused )
        {
            if ( auto * behavior_tree_component = GetBTComponentForTask( task ) )
            {
                ...
                if ( const auto * my_memory = CastInstanceNodeMemory< FAIExtTryActivateAbilityBTTaskMemory >( raw_memory ) )
                {
...
void UAIExtBTTask_TryActivateAbility::OnTaskFinished( UBehaviorTreeComponent & owner_comp, uint8 * node_memory, const EBTNodeResult::Type task_result )
{
    auto * memory = CastInstanceNodeMemory< FAIExtTryActivateAbilityBTTaskMemory >( node_memory );
    memory->Task.Reset();
...
EBTNodeResult::Type UAIExtBTTask_TryActivateAbility::AbortTask( UBehaviorTreeComponent & owner_comp, uint8 * node_memory )
{
    auto * memory = CastInstanceNodeMemory< FAIExtTryActivateAbilityBTTaskMemory >( node_memory );
    if ( auto * ai_task = memory->Task.Get() )
    {
...
UAbilitySystemComponent * UAIExtBTTask_TryActivateAbility::GetAbilitySystemComponent( UBehaviorTreeComponent & owner_comp ) const
{
...
void UAIExtBTTask_TryActivateAbility::StartTimer( UBehaviorTreeComponent & owner_comp, uint8 * node_memory )
{
...
UAIExtBTTask_TryActivateAbilityByClass::UAIExtBTTask_TryActivateAbilityByClass( const FObjectInitializer & object_initializer ) :
    Super( object_initializer )
{
    NodeName = TEXT( "Try Activate Ability By Class" );
}

FString UAIExtBTTask_TryActivateAbilityByClass::GetDetailedStaticDescription() const
{
...
void UAIExtBTTask_TryActivateAbilityByClass::SetupAITask( UAIExtAITask_ActivateAbility & ai_task, AAIController & ai_controller, UAbilitySystemComponent & asc )
{
    ai_task.Setup( ai_controller, asc, bEndWhenAbilityEnds, AbilityClass );
}

UAIExtBTTask_TryActivateAbilityByTag::UAIExtBTTask_TryActivateAbilityByTag( const FObjectInitializer & object_initializer ) :
    Super( object_initializer )
{
    NodeName = TEXT( "Try Activate Ability By Tag" );
}

FString UAIExtBTTask_TryActivateAbilityByTag::GetDetailedStaticDescription() const
{
    return AbilityTag.ToString();
}

void UAIExtBTTask_TryActivateAbilityByTag::SetupAITask( UAIExtAITask_ActivateAbility & ai_task, AAIController & ai_controller, UAbilitySystemComponent & asc )
{
    ai_task.Setup( ai_controller, asc, bEndWhenAbilityEnds, AbilityTag );
}

FAIExtBTTaskSendGameplayEventAssetSelector::FAIExtBTTaskSendGameplayEventAssetSelector() :
    AssetSource( EAIExtBTTaskSendGameplayEventAssetSource::None ),
    Asset( nullptr )
{}

UObject * FAIExtBTTaskSendGameplayEventAssetSelector::GetAsset( UBehaviorTreeComponent & owner_comp ) const
{
    switch ( AssetSource )
    {
        case EAIExtBTTaskSendGameplayEventAssetSource::None:
        {
            return nullptr;
        }
        case EAIExtBTTaskSendGameplayEventAssetSource::FromBlackboard:
        {
        ...
        case EAIExtBTTaskSendGameplayEventAssetSource::FromReference:
        {
            return Asset;
        }
        default:
...
UAIExtBTTask_SendGameplayEvent::UAIExtBTTask_SendGameplayEvent( const FObjectInitializer & object_initializer ) :
    Super( object_initializer ),
    EventMagnitude( 0 )
{
...
EBTNodeResult::Type UAIExtBTTask_SendGameplayEvent::ExecuteTask( UBehaviorTreeComponent & owner_comp, uint8 * node_memory )
{
...
FString UAIExtBTTask_SendGameplayEvent::GetDetailedStaticDescription() const
{
